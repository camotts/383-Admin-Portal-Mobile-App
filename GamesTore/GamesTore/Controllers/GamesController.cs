...
namespace GamesTore.Controllers
{
    public class GamesController : BaseApiController
    {
        ...
        public HttpResponseMessage GetGameByName(string name)
        {
            ...
            IEnumerable<GameModel> gameList = db.Games.Where(g => g.GameName.Contains(name));
            if (gameList == null)
            {
                throw new HttpResponseException(Request.CreateResponse(HttpStatusCode.NotFound));
            }
            List<GetGameDTO> sendList = new List<GetGameDTO>();
            foreach (var item in gameList)
            {
                sendList.Add(Factory.Create(item));
            }
            return Request.CreateResponse(HttpStatusCode.OK, sendList);
        }
